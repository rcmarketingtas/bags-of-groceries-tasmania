      ...
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <Accordion type="single" collapsible className="space-y-0">
          {faqs.map((faq, index) => (
            ...
            >
              <AccordionTrigger className="text-left text-base font-medium text-white hover:text-white/90 hover:no-underline [&>svg]:text-white">
                {faq.question}
              </AccordionTrigger>
              <AccordionContent className="leading-relaxed text-white/85">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
